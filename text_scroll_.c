...
void ScrollText_UpdateParam()
{
	if (scrlTxtUpdate.pause>0){
		...
		scrlTxtUpdate.pause++;
		if (scrlTxtUpdate.pause>SCRLTXT_PAUSE_COUNTER)		scrlTxtUpdate.pause=0;
	...
	if (scrlTxtUpdate.toRight)	TXTSCRLL_DBGMSG(("[%d ->>>]", scrlTxtUpdate.startIndex));
	else						TXTSCRLL_DBGMSG(("[%d <<<-]", scrlTxtUpdate.startIndex));
}
